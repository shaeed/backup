...
CRGB _led2812bs[LED2812B_NUM];
bool _changeColor = true;
#define COLOR_ON      0X0000FF
#define COLOR_OFF     0X00FF00
...
void _setColor2812b(unsigned long clr){
    if(!_changeColor) 
        return;

    fill_solid(_led2812bs, LED2812B_NUM, clr);
    delay(1);
    FastLED.show();
    _changeColor = false;
}

void _setColor2812b(unsigned char r, unsigned char g, unsigned char b){
    if(!_changeColor) 
        return;

    //fill_solid(_led2812bs, LED2812B_NUM, clr);
    for(unsigned char i=0; i< LED2812B_NUM; i++) {
        _led2812bs[i].r = r;
        _led2812bs[i].g = g;
        _led2812bs[i].b = b;
    }
...
byte _fadeSingleColor(byte clr) {
...
void _setColor2812b(unsigned long clr, unsigned char ledNo){
    _led2812bs[ledNo] = clr;
}

void _updateLeds2812b() {
    static unsigned long lastUpdate = 0;
    static byte clr = 10;
    //Start
    if(!wifiConnected()) {
        if(millis() - lastUpdate > FADE_DELAY) {
            lastUpdate = millis();
            _changeColor = true;
            clr = _fadeSingleColor(clr);
            _setColor2812b(clr, 0, 0);
        }
        //_setColor2812b(CRGB::Red);
        ...
        if(millis() - lastUpdate > FADE_DELAY) {
            ...
            _changeColor = true;            
            clr = _fadeSingleColor(clr);
            _setColor2812b(clr, 0, clr);
        }
        //_setColor2812b(CRGB::Orange);

    } else if(_changeColor) {
        unsigned long clr = 0;

        for (unsigned char id = 0; id < _relays.size(); id++) {
            clr = relayStatus(id)? COLOR_ON : COLOR_OFF;
            _setColor2812b(clr, id);
        }

        DEBUG_MSG_P(PSTR("[RELAY] FastLed color chnaged\n"));
        FastLED.show();
        _changeColor = false;
    }
}
...
void relaySetup() {
    ...
    //Leds 2812b
    #if LED2812B_NUM
        pinMode(LED2812B_PIN, OUTPUT);
        FastLED.addLeds<STRIP_TYPE, LED2812B_PIN, COLOR_MODE>(_led2812bs, LED2812B_NUM);
